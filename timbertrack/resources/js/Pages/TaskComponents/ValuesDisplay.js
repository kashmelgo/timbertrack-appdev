...
const ValueDisplay= ({task, employees, employee, setEmployee})=>{

    const deleteHandler = () =>{
        setEmployee(employee.filter((el) => el.id !== employees.id));
    };
    ...
    return(
        ...
        className={`bg-white p-5 ${employees.completed? "opacity-50 line-through hover:no-underline hover:opacity-100": ""} `}
        >
            {task}
            <br/>
           <button onClick={completeStatus}  className="rounded-full h-7 w-7 border-2 border-black-200 mx-2 hover:border-black">
            <svg class="h-5 w-5 text-black mx-0.5"  width="24" height="24" viewBox="0 0 24 24"
             stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            <path stroke="none" d="M0 0h24v24H0z"/>

            {(employees.completed)?<line x1="18" y1="6" x2="6" y2="18" />
              :<path d="M5 12l5 5l10 -10" /> }
            {(employees.completed)?<line x1="6" y1="6" x2="18" y2="18" />: undefined}
...
const ValueDisplay= ({task, List, employee, usertype})=>{
